	...
	CancellationToken cancellationToken)
{
	...
	private const string BASE_NAMESPACE = "Net.FracturedCode.Infisical";
	public async Task GenerateFile(string generationName, Expression<Func<CSharpClientGeneratorSettings, bool>> generationTargetExpression, IEnumerable<string> partialUsingNamespaces)
	{
		...
		settings.CSharpGeneratorSettings.Namespace = $"{BASE_NAMESPACE}.{generationName}";
		settings.AdditionalNamespaceUsages = partialUsingNamespaces
			.Select(x => $"{BASE_NAMESPACE}.{x}")
			.ToArray();
		settings.ParameterNameGenerator = new InfisicalParameterNameGeneration();
